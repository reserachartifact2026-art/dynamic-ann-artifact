...
struct WorkloadEvent {
    ...
    std::vector<unsigned> groundtruth;  // For QUERY: embedded GT from workload generator

    WorkloadEvent() : type(EVENT_QUERY), timestamp(0), pointId(0), queryId(0) {}

    bool hasGroundtruth() const { return !groundtruth.empty(); }
};
...
public:
    ...
    static std::vector<unsigned> extractUnsignedArray(const std::string& json, const std::string& key);
};
