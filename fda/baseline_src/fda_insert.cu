...
void allocateInsertBatchBuffers(InsertBatchBuffers* b, unsigned maxBatch) {
    b->maxBatch = maxBatch;
    gpuErrchk(cudaMallocHost(&b->h_vectors,  (size_t)maxBatch * D * sizeof(float)));
    gpuErrchk(cudaMallocHost(&b->h_pointIds, (size_t)maxBatch * sizeof(unsigned)));
    gpuErrchk(cudaMalloc(&b->d_vectors,       (size_t)maxBatch * D * sizeof(float)));
                                              ...
                                              unsigned        batchSize)
{
    ...
    const float* src = d_vectors + (size_t)i * D;
...
static float l2Dist(const float* a, const float* b) {
    float d = 0.0f;
...
__device__ __forceinline__
static float l2DistBlock(const float* a, const float* b, float* smemAccum)
{
    float partial = 0.0f;
...
// ============================================================================
// THE ROUTED REVERSE-EDGE KERNEL.
// One thread per (target, source) pair. Routes by target region:
//   target < ltiCount  → append to DeltaEdgeBuffer
//   target ≥ ltiCount  → locked append into unified d_graph
// ============================================================================
// Block-cooperative version: ONE BLOCK per (target, source) pair.
// Thread 0 acquires the per-node lock, all threads cooperate on the α-RNG
// distance computations inside the locked region. The cooperative l2 is
// ~30× faster per call than single-thread serial l2, which directly
// reduces the lock hold time on hub vertices.
__global__ void appendReverseEdgesRoutedKernel(
    uint8_t*  d_graph,
    uint8_t*  d_locks,
    unsigned* d_deltaCount,
    unsigned* d_deltaStorage,
    unsigned  immutableEnd,    // = rwBase: < this → delta, ≥ → locked RW append
    unsigned* d_revSrc,
    unsigned* d_revDst,
    unsigned* d_revCount,
    float     alpha)
{
    ...
    // LTI/Temp-target path: append to delta buffer (drop on overflow).
    // The merge will fold these into permanent neighbor lists.
    if (target < immutableEnd) {
        if (threadIdx.x == 0) {
            unsigned slot = atomicAdd(&d_deltaCount[target], 1u);
            if (slot < FDA_DELTA_CAP) {
                d_deltaStorage[(size_t)target * FDA_DELTA_CAP + slot] = source;
            } else {
                atomicSub(&d_deltaCount[target], 1u);
            }
        }
        return;
    }

    // RW-target path: locked append. Block holds the lock; on overflow, all
    // threads cooperate on α-RNG distance computations.
    ...
    unsigned B, float alpha)
{
    ...
    unsigned slot = d_pointIds[i];
    const float* myVec = d_newVectors + (size_t)i * D;
                            ...
                            bool            d_vectorsPreLoaded)
{
    ...
    if (!d_vectorsPreLoaded) {
        std::memcpy(bufs->h_vectors, h_newVectors,
                    (size_t)batchSize * D * sizeof(float));
    }
    ...
    writeNewVectorsAndZeroDegree<<<batchSize, 128, 0, stream>>>(
    ...
    // Fused locked greedy v2 — single kernel launch per batch.
    // Fine-grained short-term per-node lock (acquire → snapshot → release).
    // Entry point: idx->rwMedoid for RW-only insert greedy.
    (void)d_deleted;
    greedySearchFusedLockedPrealloc(
        view.d_graph,
        view.d_locks,
        bufs->d_vectors,
        bufs->d_visitedSets,
        bufs->d_visitedSetCounts,
        /*d_visitedSetDists*/ nullptr,
        /*batchStart*/ 0,
        batchSize, searchL,
        /*entryPoint*/ idx->rwMedoid,
        &bufs->gsBuffers,
        stream);

    robustPruneBatchKernel<<<batchSize, 128, 0, stream>>>(
        view.d_graph,
        bufs->d_vectors,
        bufs->d_visitedSets,
        bufs->d_visitedSetCounts,
...

    {
        ...
        appendReverseEdgesRoutedKernel<<<maxPairs, 128, 0, stream>>>(
            view.d_graph, view.d_locks,
            view.d_deltaCount, view.d_deltaStorage,
            view.immutableEnd,
            bufs->d_revEdgeSrc, bufs->d_revEdgeDst, bufs->d_revEdgeCount,
            alpha);
    }
                           ...
                           bool d_vectorsPreLoaded)
{
    ...
    gpuErrchk(cudaMemsetAsync(bufs->d_visitedSetCounts, 0,
                              (size_t)batchSize * sizeof(unsigned), stream));
    // Phase B re-inserts into NEW LTI → entry point = ltiMedoid.
    (void)d_deleted;
    greedySearchFusedLockedPrealloc(
        view->d_graph,
        view->d_locks,
