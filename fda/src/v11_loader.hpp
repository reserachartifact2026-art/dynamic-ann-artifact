...
inline VecStore<T> loadBin(const std::string& path, size_t max_rows = 0) {
    ...
    // FIX: trust file size over header. Some bases carry a stale/bogus row count
    // (e.g. msturing-100m header n=1e9) -> would over-read ("short data"). Clamp n
    // to what the file actually holds.
    {
        long _cur = std::ftell(f);
        std::fseek(f, 0, SEEK_END);
        long _end = std::ftell(f);
        std::fseek(f, _cur, SEEK_SET);
        if (dim > 0) {
            size_t _drows = (size_t)(_end - _cur) / (sizeof(T) * (size_t)dim);
