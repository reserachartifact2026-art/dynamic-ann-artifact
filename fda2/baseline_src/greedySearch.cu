...
// DEBUG instrumentation: tag set by callers (0=insert, 1=search, 2=delete) so we
// can compare greedy iteration counts per op-type. Remove after diagnosis.
int  g_greedyTag = -1;
long g_greedyIterSum[3]  = {0, 0, 0};
long g_greedyCallCnt[3]  = {0, 0, 0};
...
__global__ void initializeWorklist(uint8_t *d_graph,
                                   ...
                                   unsigned searchL,
                                   const unsigned *d_querySlots = nullptr) {
    unsigned queryID = blockIdx.x;
    unsigned tid = threadIdx.x;
    ...
    // Query vector source: read in place from the graph entry of
    // d_querySlots[queryID] when given (DELETE path — no gather/copy), else the
    // legacy contiguous layout d_queryVecs + D*queryID.
    GVEC *queryVec = d_querySlots
        ? (GVEC*)(d_graph + graphEntrySize * (size_t)d_querySlots[queryID])
        : d_queryVecs + D * queryID;
...
// ============================================================================
// filterNeighborsLocked — spec-compliant adjacency read under per-node lock.
//
// Used by INSERT greedy. Per the spec:
...
// ============================================================================
// greedySearchLockedPrealloc — spec INSERT path.
...
// ============================================================================
// greedySearchNoSyncPrealloc — spec SEARCH path.
// Same as Versioned but uses filterNeighborsNoSync (direct read, no locks
// or version checks). Spec: "No need to take locks while reading the
...
void greedySearchNoSyncPrealloc(uint8_t *d_graph,
    ...
    *h_nextIter = true;
    ...
    // DEBUG: accumulate greedy iteration count by op-type tag.
    if (g_greedyTag >= 0 && g_greedyTag < 3) {
        g_greedyIterSum[g_greedyTag] += iter;
        g_greedyCallCnt[g_greedyTag] += 1;
    }
}
                                        ...
                                        unsigned searchL)
{
    ...
    if (tid == 0) {
        ...
        neighborCount = 0;
        Dist dist = 0;
        // Use the RUNTIME medoid (set via setRuntimeMedoid), not the compile-time
        // MEDOID. The sequential model re-selects a live entry point when the
        // current medoid is deleted (sliding-window deletes node 5000); starting
        // the fused greedy at a tombstoned node would collapse recall.
        unsigned med = g_runtimeMedoid;
        Vec *medoidVec = (Vec*)(d_graph + (size_t)med*graphEntrySzT<Idx, Vec>);
        for (int i = 0; i < D; i++) {
