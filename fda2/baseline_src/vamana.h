...
// or neighbor-driven (R / R+P / R+MAX_L). See WALKTHROUGH.md for the
...
// Graph vector storage type — parameterized, NOT hardcoded. Default float; build
// with -DFDA2_STORE_T=uint8_t for native-uint8 datasets (e.g. sift100m) to cut the
// per-node footprint (D*4 -> D*1 bytes) so 100M fits in an 80GB A100. Distance math
// still accumulates in float (GVEC auto-promotes), so recall is unchanged.
#ifndef FDA2_STORE_T
...
// ============================================================================
// FreshDiskANN spec-compliant greedy wrappers.
//
// greedySearchLockedPrealloc  — INSERT path. Per-node binary lock acquired
//                               around adjacency reads (spec: "Before reading
//                               the adjacency list of a node, use the lock.").
// greedySearchNoSyncPrealloc  — SEARCH path. No locks, no version check (spec:
...
void greedySearchNoSyncPrealloc(uint8_t *d_graph,
                                 ...
                                 GreedySearchBuffers* buffers,
                                 cudaStream_t stream = 0,
                                 // When non-null, query vector for item i is read in
                                 // place from the graph entry of slot d_querySlots[i]
                                 // (no gather). Used by the DELETE path.
