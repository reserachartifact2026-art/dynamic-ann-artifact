...
bool FreshBANG<T>::BuildIndex(const T* base_vectors, uint32_t num_basevectors)
{
  ...
  const bool should_try_load = (mode == FreshBANGMode::kLoad) ||
                               (mode == FreshBANGMode::kLegacy && has_index_on_disk);

  if (should_try_load) {
    if (!has_index_on_disk) {
      std::cerr << "[FreshBANG::BuildIndex] Error: load mode requires an existing index file at '"
                << impl->index_file() << "'." << std::endl;
      return false;
    }
    std::cout << "[FreshBANG::BuildIndex] Existing index found on disk; loading instead of building"
              << std::endl;
    try {
      algo_obj->load(impl->index_file());
      std::cout << "[FreshBANG::BuildIndex] load() completed" << std::endl;
      if (cudaDeviceSynchronize() != cudaSuccess) {
        std::cerr << "[FreshBANG::BuildIndex] Warning: cudaDeviceSynchronize failed after load."
                  << std::endl;
      }
      log_gpu_mem("after_load");
      impl->set_has_live_index(true);

      if (index_path.has_parent_path()) {
        std::cout << "[FreshBANG::BuildIndex] Existing index load path kept directory ready: "
                  << index_path.parent_path() << std::endl;
      }
      return true;
    } catch (const std::exception& e) {
      std::cerr << "[FreshBANG::BuildIndex] Error: failed to load existing index '"
...
bool FreshBANG<T>::SetInsertParams()
{
  ...
  auto* impl = static_cast<cuvs::bench::detail::FreshBANGInner<T>*>(m_pImpl);
  if (impl->mode() == FreshBANGMode::kBuild || impl->mode() == FreshBANGMode::kLoad) {
...
void FreshBANG<T>::Cleanup()
{
  ...
  if (impl->mode() == FreshBANGMode::kLoad) {
    std::cout << "[FreshBANG::Cleanup] Load mode: keeping read-only index file on disk" << std::endl;
    delete impl;
    m_pImpl = nullptr;
    return;
  }
...
void FreshBANG<T>::SaveIndex()
{
  ...
  if (impl->mode() == FreshBANGMode::kLegacy || impl->mode() == FreshBANGMode::kBuild) {
